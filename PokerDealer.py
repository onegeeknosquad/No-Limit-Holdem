...
from PokerEval import to_string
from Deck import r_deck, dic_deck, board_to_int



class PokerDealer:
    def __init__(self,bb=2,sb=1,pot=0,street=0, board=[]):
        self.players = 0
        self.players_list = {}
        self.button_seat = 0
    ...
    def sit_player(self, buy_in, hand=None, human=False):
        self.players_list[self.players] = PokerPlayer(self.players, buy_in, hand, human)
        self.players += 1
    ...
    def player_actions(self):
        ...
        while not all_acted:
            ...
            all_acted = all([player.acted for player in self.players_list.values()])
        if all([player.has_folded() for player in self.players_list.values()]):
            return True
        self.street += 1
        if self.street == 4:
    ...
    def new_hand(self):
        ...
        self.player_actions()
        print(to_string(self.players_list[0].hand,self.players_list[1].hand,self.board))
        if showdown(self.players_list[0].hand,self.players_list[1].hand,self.board):
            return self.pot
